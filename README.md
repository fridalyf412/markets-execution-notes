# Markets Execution Notes (FX) — v1

A lightweight, repeatable daily routine to build execution intuition in liquid markets:
**what moved, why it likely moved, what risks matter next, and what to watch for execution.**

## What this is (and is not)
- ✅ A disciplined template for summarizing market drivers, risks, and key levels
- ✅ Practice clear, client-ready communication under time pressure
- ❌ Not a “trading strategy,” not an alpha claim, not investment advice

## Contents
- `method.md` — scope, rules, and sources
- `template.md` — daily note template
- `notes/` — daily notes (one page each)

## Current watchlist (FX)
EURUSD • GBPUSD • USDJPY • DXY (optional context: US 2Y/10Y)

## Disclaimer
For educational purposes only. No solicitation, recommendation, or investment advice.
